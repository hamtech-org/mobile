...
export default function EditPostEditorScreen() {
  ...
  useEffect(() => {
    ...
    // Existing URLs become 'remote' items (already uploaded)
    setMediaItems((post.mediaUrls ?? []).map((url) => ({ kind: "remote" as const, url })));
  }, [post]);

  const [uploadMedia, { isLoading: uploading }] = useUploadMediaMutation();
  const [updatePost, { isLoading: updating }] = useUpdatePostMutation();

  /** Stage assets locally – NO upload yet. */
    ...
    <SafeAreaView className="flex-1 bg-background" edges={["top"]}>
      ...
      >
        ...
        <View className="flex-row items-center justify-between border-b border-border/40 px-4 py-3">
          ...
          <Text className="text-lg font-bold">Chỉnh sửa bài viết</Text>
          <Pressable
